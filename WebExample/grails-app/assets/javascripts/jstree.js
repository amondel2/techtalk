			...
			var w = null,
				...
				func = function (data, undefined) {
					...
					var dat = data.dat,
						...
						sel = data.sel,
						tmp, i, j, rslt,
						parse_flat = function (d, p, ps,childType) {
							if(!ps) { ps = []; }
							else { ps = ps.concat(); }
							if(p) { ps.unshift(p); }
							...
							for(i = 0, j = tmp.children.length; i < j; i++) {
								c = parse_flat(m[tmp.children[i]], tmp.id, ps, m[tmp.children[i]] && m[tmp.children[i]].hasOwnProperty('childType') ? m[tmp.children[i]].childType : '');
								e = m[c];
						...
						parse_nest = function (d, p, ps,childType) {
							if(!ps) { ps = []; }
							else { ps = ps.concat(); }
							if(p) { ps.unshift(p); }
							...
							if(d && d.children && d.children.length) {
								for(i = 0, j = d.children.length; i < j; i++) {
									c = parse_nest(d.children[i], tmp.id, ps, d.children[i] && d.children[i].hasOwnProperty('childType') ? d.children[i].childType : '' );
									e = m[c];
									tmp.children.push(c);
									if(e.children_d.length) {
										tmp.children_d = tmp.children_d.concat(e.children_d);
					...
					if(dat.length && dat[0].id !== undefined && dat[0].parent !== undefined) {
						...
						for(i = 0, j = p.children.length; i < j; i++) {
							tmp = parse_flat(m[p.children[i]], par, p.parents.concat(), m[p.children[i]].childType);
							dpc.push(tmp);
							if(m[tmp].children_d.length) {
								dpc = dpc.concat(m[tmp].children_d);
							}
						...
						for(i = 0, j = dat.length; i < j; i++) {
							tmp = parse_nest(dat[i], par, p.parents.concat(),dat[i].childType);
							if(tmp) {
								chd.push(tmp);
								dpc.push(tmp);
								if(m[tmp].children_d.length) {
